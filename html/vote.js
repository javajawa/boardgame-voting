...
function game_to_tr(game) {
    return tr(
        ...
        game.realm.realtime ? td((1 - game.voted).toString(), { class: "border vote", click: toggleVote }) : null,
        game.realm.passnplay ? td((1 - game.avoted).toString(), { class: "border avote", click: toggleVote }) : null
    );
}
...
function table_headers(realm) {
    ...
    return tr(Object.entries(headings).map(h => th(...h, { click: sortTable })));
}

Promise.all([fetch("games.json").then(r => r.json()), fetch("me").then(r => r.json())])
    .then(r => {
        const games = r[0];
        const me = r[1];
    ...
    .then(games =>
        table(thead(table_superheaders(games[0].realm), table_headers(games[0].realm)), tbody(games.map(game_to_tr)))
    )
